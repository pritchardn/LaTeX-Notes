...
\node (NW) [anchor=north west] at (current page.north west){\pgfornament[width=4cm] {41}};
\node (NE) [anchor=north east] at (current page.north east){\pgfornament[width=4cm, symmetry=v]{41}};
\node (SW) [anchor=south west] at (current page.south west){\pgfornament[width=4cm, symmetry=h]{41}};
\node (SE) [anchor=south east] at (current page.south east){\pgfornament[width=4cm, symmetry=c]{41}};
\foreach \i in {-4,0,4}
...
\textit{\Large International Centre for Radio Asrtonomy Research} 
...
\begin{figure}[!h]
    \centering
    \includegraphics[width = 3cm, height= 3cm]{Notes/resource/ICRAR_large_logo_colour.png}%% include the university icon here
\end{figure}
\vspace{0.3\baselineskip} 


{\large Edited by\\  Nicholas Pritchard}
\end{titlepage}
